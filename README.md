...
- OpenSees does not combine constraints (multiple 'fix' commands on the same node). As a result :
  (a) Intersection joints that belong on two lines with different constraints should be constrained seperately.
  (b) Intersection lines that belong on two surfaces with different constraints should be constrained seperately.
